...
def _capture_stub(*, frame: Image.Image, hwnd: int = 123):
    ...
    capture._win32gui = SimpleNamespace(
        GetWindowRect=lambda _hwnd: (0, 0, frame.width, frame.height)
    )

    class FakeGraphicsCapture:
        def __init__(self, received_hwnd, capture_size):
            assert received_hwnd == hwnd
            assert capture_size == (100, 60)
